# 기술 리포트: 한국어 금융 및 법률 리랭커 PEFT

> ⚠️ 이 리포트의 성능 수치는 철회되었습니다 (2026-08). [`EVAL_AUDIT.md`](./EVAL_AUDIT.md)

## 1. 서론 (Introduction)

본 프로젝트는 한국어 금융 및 법률 도메인에 특화된 고성능 리랭커(Reranker) 모델을 개발하는 것을 목표로 하였습니다. 금융 및 법률 문서는 전문 용어의 비중이 높고 문맥적 미묘함이 중요하여, 범용 모델만으로는 높은 검색 정확도를 보장하기 어렵습니다. 이를 해결하기 위해 PEFT(Parameter-Efficient Fine-Tuning) 기술과 전략적 데이터 증강 기법을 결합한 파이프라인을 구성하였습니다.
...
> ⚠️ 위 실패 사례는 **검증셋에서 추출**된 것이며, 이를 학습한 모델을 같은 검증셋으로
> 재평가했습니다. EVAL_AUDIT.md 결함 2에 해당합니다.

3.  **효율성**: 이 전략을 통해 전체 재학습 시 8시간 이상 소요되던 시간을 **약 30분**으로 단축했습니다.

## 5. 평가 결과 (Evaluation Results) — ⚠️ 철회됨, EVAL_AUDIT.md 참조

아래 수치는 철회되었습니다. 측정 방식의 결함은 EVAL_AUDIT.md를 참조하세요.
무엇을 주장했었는지를 남기기 위해 표는 삭제하지 않습니다.
